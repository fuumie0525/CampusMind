"""
CampusMind 校园多 Agent 咨询服务系统 — FastAPI 入口
...
def _llm_cfg() -> Dict[str, Any]:
    ...
    CampusMind 使用 Anthropic SDK 的消息接口，并支持配置 Anthropic-compatible
    网关。DEEPSEEK_API_KEY/LLM_* 为推荐变量，旧 ANTHROPIC_* 变量继续兼容。
    """
    key = os.getenv("DEEPSEEK_API_KEY") or os.getenv("ANTHROPIC_API_KEY", "")
    if not key:
        raise RuntimeError("未设置 DEEPSEEK_API_KEY 或 ANTHROPIC_API_KEY")
    cfg: Dict[str, Any] = {
        "api_key": key,
        "model": os.getenv("LLM_MODEL") or os.getenv("ANTHROPIC_MODEL", "deepseek-v4-flash"),
    }
...
async def lifespan(app: FastAPI):
    ...
    skills_dir = os.getenv("CAMPUSMIND_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills"))
    _skill_manager = SkillManager(
        root_dir=skills_dir,
        max_prompt_chars=int(os.getenv("CAMPUSMIND_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    _skill_manager.load()
    ...
    # 校园业务工具：学生事务、网络自查、校园卡和宿舍报修。
    from mcp.campus_tools import register_campus_tools
    register_campus_tools(_tool_manager)
    ...
    logger.info("CampusMind 已就绪")
    yield

    await _monitor.stop()
    logger.info("CampusMind 已关闭")


# ── FastAPI ───────────────────────────────────────────────────────────────────
app = FastAPI(
    title="CampusMind 校园智能咨询",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
)
...
async def chat(req: ChatRequest):
    """
    主对话接口。完整流程：
      记忆读取 → RAG/校园工具调用 → 意图识别 → Agent 路由 → 执行 → 记忆写入
    """
    ...
    (knowledge_text, knowledge_used), (tool_text, tools_used) = await asyncio.gather(
        _build_knowledge_context(req.message),
        _build_campus_tool_context(req.message),
    )
...
async def _build_campus_tool_context(message: str) -> tuple[str, List[str]]:
    """按关键词选择校园业务工具并并行调用，将结果注入 Agent 上下文。"""
    if _tool_manager is None:
        return "", []

    from mcp.campus_tools import select_tool_calls

    calls = select_tool_calls(message)
    if not calls:
        return "", []

    tasks = [
        _tool_manager.call(name, params, context={"source": "chat"})
    ...
    if not used:
        return "", []
    parts.append("工具结果只用于提供流程与自助排查，不代表已经完成后台查询、审批、充值、挂失或报修。")
    return "\n".join(parts), used
...
async def _build_knowledge_context(message: str, top_k: int = 3) -> tuple[str, bool]:
    ...
    try:
        ...
        if not used:
            return "", False
        parts.append("请优先依据以上知识库内容回答；如果知识库内容不足，再结合校园综合服务能力说明。")
        return "\n".join(parts), True
...
def _should_use_knowledge(message: str) -> bool:
    ...
    if msg in greetings:
        return False
    business_keywords = [
        "校园政策", "规定", "奖学金", "助学金", "请假", "缓考", "学籍", "在读证明", "学生证",
        "校园网", "wifi", "vpn", "认证", "断网", "网络", "校园卡", "饭卡", "充值", "挂失", "补卡",
        "扣费", "缴费", "宿舍", "寝室", "报修", "门禁", "空调", "办事大厅", "教务", "一卡通",
    ]
...
async def add_knowledge(body: BatchDocInput):
    ...
    {
      "documents": [
        {"title": "奖学金评定办法", "content": "申请人应按当年度学校通知提交材料，并以学院审核与公示结果为准..."},
        {"title": "校园网使用指南", "content": "校园网认证失败时可先检查账号状态、认证页面和网络配置..."}
      ]
    }
    ```
    """
...
async def _cli():
    print(BANNER)
    print("CampusMind CLI — 输入 quit 退出\n")
    ...
    skill_manager = SkillManager(
        root_dir=os.getenv("CAMPUSMIND_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills")),
        max_prompt_chars=int(os.getenv("CAMPUSMIND_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    skill_manager.load()
    ...
    while True:
        ...
        print(f"\nCampusMind [{result.agent_type.value}]: {result.response}\n")
